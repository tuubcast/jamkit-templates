...
function show_comments() {
    if (document.value("playing.video-id")) {
        ...
        controller.action("toast", { "message":"재생 중인 영상이 없습니다." })
    }
}
...
function shuffle_music() {
    if (document.value("playing.video-id")) {
        ...
        controller.action("toast", { "message":"음악 재생 순서를 변경합니다." })
    } else {
        controller.action("toast", { "message":"재생 중인 영상이 없습니다." })
    }
}
...
function next_music() {
    if (document.value("musics.loaded")) {
        ...
        controller.action("toast", { "message":"아직 음악 목록을 불러오지 못했습니다." })
    }
}
...
function repeat_music() {
    ...
    controller.action("toast", { "message":"반복 재생이 설정되었습니다." })
}

function repeat_music_done() {
    view.object("showcase.musics").action("repeat-done")
    controller.action("toast", { "message":"반복 재생이 해제되었습니다." })
}
